...
//	{
	...
	Format(CHAT_TAG, sizeof(CHAT_TAG), "[DETRAN]")
	Cvar_Prefix = CreateConVar("sm_blockbuilder_prefix",	"[DETRAN]", "A Prefix used by messages within Blockbuilder...")
...
//		}
	...
	PrintToChatAll("\x03%s\x04 %d blocos e %d pares de teleportes foram salvos.", CHAT_TAG, blocks, teleporters);
	PrintToServer("%d blocos e %d teleportes salvos", blocks, teleporters);
	...
	for(new a=MaxClients+1;a<2048;++a)
	{
		...
		if(g_iBlocks[a]==10)
		{
			...
			if(fPos[0] - 20.0 < fPos2[0] < fPos[0] + 20.0 && fPos[1] - 20.0 < fPos2[1] < fPos[1] + 20.0 && fPos[2] - 60.0 < fPos2[2] < fPos[2] + 60.0)
			{
				...
				if(iTeam==2)
					PrintToChatAll("\x03%s\x04 %N matou o time Contra-Terrorista.", CHAT_TAG, client);
				else if(iTeam==3)
					PrintToChatAll("\x03%s\x04 %N matou o time Terrorista.", CHAT_TAG, client);
...
public Handle:CreateMainMenu(client)
{
	...
	SetMenuTitle(menu, "[DETRAN] Blockmaker");
		...
		if(fSpeed <= 50.0){
			...
			if (TR_DidHit()){
				...
				if(IsValidBlock(iEnt)){
					if(g_iBlocks[iEnt] == 1 || g_iBlocks[iEnt] == 11 || g_iBlocks[iEnt] == 28 || g_iBlocks[iEnt] == 35){
						g_bTriggered[iEnt]=true;
						CreateTimer(0.1, StartNoBlock, iEnt);
						
						PrintToChat(iGracz, "\x03%s\x04 Você foi solto!", CHAT_TAG);
					}
				}
			}
		}
...
public touchGracza(client, sciana){
	...
	if(g_hSprawdz[client] == INVALID_HANDLE){ 
		g_hSprawdz[client] = CreateTimer(1.2, sprawdzBh, client);
	}
	if(!(1 <= client <= MaxClients)) return;
	if(!(IsClientInGame(client) && IsPlayerAlive(client))) return;
	...
	//g_hSprawdz[client] = CreateTimer(3.0, sprawdzBh, client);
}
...
public OnStartTouch(ent1, ent2)
{
	...
	if(GetEntityFlags(client) & FL_ONGROUND && GetEntPropEnt(client, Prop_Send, "m_hGroundEntity") == block)
	{
	...
	if(g_iBlocks[block]==24 || g_iBlocks[block]==53 || g_iBlocks[block]==82 || g_iBlocks[block]==111)
	{
		if(!g_bRandomCantUse[client])
		{
			...
			else if(random == 6)
			{
				...
				PrintToChatAll("\x03%s\x04 %N pegou uma Deagle", CHAT_TAG, client);
			}
			else if(random == 7)
			{
				new ent = -1;
				...
				SetEntProp(ent, Prop_Data, "m_iClip2", 1);
				SetEntData(client, g_iAmmo+(GetEntData(ent, g_iPrimaryAmmoType)<<2), 0, 4, true);
				PrintToChatAll("\x03%s\x04 %N pegou uma AWP", CHAT_TAG, client);
			}
			else if(random == 8)
			{
