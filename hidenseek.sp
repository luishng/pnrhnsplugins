	...
	*/
	...
	#define FLASHBANG_CHANCE			  "0"
	#define MOLOTOV_CHANCE				"0"
	#define SMOKE_CHANCE				  "0.0"
	#define DECOY_CHANCE				  "0"
	#define HE_CHANCE					 "0.0"
	#define FLASHBANG_MAXIMUM_AMOUNT	  "0"
	#define MOLOTOV_MAXIMUM_AMOUNT		"0"
	#define SMOKE_MAXIMUM_AMOUNT		  "0"
	#define DECOY_MAXIMUM_AMOUNT		  "0"
	#define HE_MAXIMUM_AMOUNT			 "0"
	#define COUNTDOWN_FADE				"1"
	#define NO_FLASH_BLIND				"2"
	#define BLOCK_JOIN_TEAM			   "0"
	#define ATTACK_WHILE_FROZEN		   "1"
	#define FROSTNADES					"1"
	#define SELF_FREEZE				   "1"
	...
	#define COUNTDOWN_COLOR		{0,0,0,250}
	...
	public OnPluginStart()
	{
		...
		AutoExecConfig(true, "hidenseek1");
	...
	public Action:FirstCountdownMessage(Handle:hTimer, any:iClient)
	{
		...
		if(IsClientInGame(iClient))
			PrintHintText(iClient, "<font size='20'color='#ADD8E6'><b>\n  Os <font size='20'color='#CABA7A'>Terroristas <font size='20'color='#ADD8E6'>têm %d segundo%s para se esconder!</b>", iCountdownTimeFloor, (iCountdownTimeFloor == 1) ? "" : "s");
	}
		...
		if(g_iCountdownCount < g_fCountdownTime) {
			for(new iClient = 1; iClient < MaxClients; iClient++) {
				if(IsClientInGame(iClient)) {
					...
					PrintHintText(iClient, "<font size='20'color='#ADD8E6'><b>\n  Os <font size='20'color='#CABA7A'>Terroristas <font size='20'color='#ADD8E6'>têm %d segundo%s para se esconder!</b>", iTimeDelta, (iTimeDelta == 1) ? "" : "s");
				}
			}
			return Plugin_Continue;
		}
			...
			for(new iClient = 1; iClient < MaxClients; iClient++) {
				if(IsClientInGame(iClient))
					PrintHintText(iClient, "<font size='20'color='#00FF00'><b>\n  E a caçada começa!</b>");
			}
	...
	public Action:Command_ToggleKnife(iClient, args)
	{
		if(iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient)) {
			...
			CPrintToChat(iClient, "{green}[pNr]{default} Sua faca está %s.", g_baToggleKnife[iClient] ? "Visível" : "Invisível");
			...
			if(!(StrEqual("sv_airaccelerate", sConVarName) && !g_iAirAccelerate)) {
				for(new i = 0; i < sizeof(g_saProtectedConVars); i++) {
					if(StrEqual(g_saProtectedConVars[i], sConVarName) && StringToInt(sNewValue) != g_iaForcedValues[i]) {
						...
						PrintToServer("%s is a protected CVAR.", sConVarName);
					}
				}
			}
		}
		...
		if(iTeam == CS_TEAM_T || iTeam == CS_TEAM_CT) {
			if(iChosenTeam == JOINTEAM_T || iChosenTeam == JOINTEAM_CT || iChosenTeam == JOINTEAM_RND) {
				if(IsPlayerAlive(iClient)) {
					CPrintToChat(iClient, "{green}[pNr]{default} Você não pode trocar de time enquanto estiver vivo.");
					return Plugin_Stop;
				}
					...
					CPrintToChat(iClient, "{green}[pNr]{default} Você não pode trocar de time enquanto eles estão balanceados.");
					return Plugin_Stop;
				}
			}
			else if(iChosenTeam == JOINTEAM_SPEC) {
				if(IsPlayerAlive(iClient)) {
					PrintToConsole(iClient, "{green}[pNr]{default} Você não pode ir para espectador enquanto estiver vivo.");
					return Plugin_Stop;
				}
				else {
				...
				PrintToConsole(iClient, "{green}[pNr]{default} You are trying to join an invalid team.");
				return Plugin_Stop;
			}
		}
			...
			if(iDelta > iLimitTeams || -iDelta > iLimitTeams) {
				...
				CPrintToChat(iClient, "{green}[pNr]{default} Você foi direcionado ao time %s", (g_iaInitialTeamTrack[iClient] == CS_TEAM_T) ? "T" : "CT");
	...
	public Action:Command_Kill(iClient, const String:sCommand[], iArgCount)
	{
		...
		if (!g_bBlockConsoleKill || iClient == 0 || iClient > MaxClients)
			return Plugin_Continue;
		PrintToConsole(iClient, "{green}[pNr]{default} You are not allowed to use the kill command.");
		return Plugin_Stop;
	}
	...
	public Action:OnRoundEnd(Handle:hEvent, const String:name[], bool:dontBroadcast)
	{
		...
		if(iWinningTeam == CS_TEAM_T) {
			if(!g_iMaximumWinStreak || ++g_iTWinsInARow < g_iMaximumWinStreak)
				CPrintToChatAll("{green}[pNr] {gold}Terroristas {default}venceram.");
			else {
				SwapTeams();
				g_iTWinsInARow = 0;
				//Set the team scores
				...
				if(g_iMaximumWinStreak)
					CPrintToChatAll("{green}[pNr] {gold}Terroristas {default}venceram vários rounds seguidos. Times foram {purple}misturados{default}.");
			}
		...
		else if(iWinningTeam == CS_TEAM_CT)
		{
			...
			SwapTeams();
			CPrintToChatAll("{green}[pNr] {blue}Contra-terroristas {default}venceram. Os times foram {purple}trocados{default}.");
			g_iTWinsInARow = 0;
			//Set the team scores
	...
	stock Freeze(iClient, Float:fDuration, iType, iAttacker = 0)
	{
		...
		//MENSAGEM QUE APARECE NO CHAT PARA OS CTS
		if(iAttacker == 0) {
			// CPrintToChat(iClient, "{green}[pNr]{default} Você está {grey2}apagado {default}por %.1f segundos.", fDuration);
		}
		else if(iAttacker == iClient) {
			// CPrintToChat(iClient, "{green}[pNr]{default} Você se {blue}congelou {default}por %.1f segundos, hurrdurr.", fDuration);
		}
			...
			if(IsClientInGame(iAttacker)) {
				...
				// CPrintToChat(iClient, "{green}[pNr]{default} Você foi {blue}congelado {default}por %s por %.1f segundos.", sAttackerName, fDuration);
			}
		}
		if(g_baFrozen[iClient]) {
			if(g_haFreezeTimer[iClient] != INVALID_HANDLE) {
	...
	public Action:Unfreeze(Handle:hTimer, any:iClient)
	{
		if(iClient && IsClientInGame(iClient)) {
			if(g_baFrozen[iClient]) {
				...
				// if(IsPlayerAlive(iClient))
					// CPrintToChat(iClient, "{green}[pNr]{default} Você está livre agora.");
			}
			else
	...
	public Action:UnfreezeCountdown(Handle:hTimer, any:iClient)
	{
		if(iClient && IsClientInGame(iClient)) {
			...
			g_haFreezeTimer[iClient] = INVALID_HANDLE;
			// if(IsPlayerAlive(iClient))
				// CPrintToChat(iClient, "{green}[pNr]{default} O round começou.");
		}
		return Plugin_Continue;
	}
