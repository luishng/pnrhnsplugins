...
#define VERSION "1.0"
#define ADMIN_LEVEL ADMFLAG_RCON
#define DEBUG 0
...
new const String:LEETJUMP_FULL_SOUND_PATH[] = "sound/jumpstats/godlike.mp3";
new const String:LEETJUMP_RELATIVE_SOUND_PATH[] = "*jumpstats/godlike.mp3";
new const String:LEETJUMP_RAMPAGE_FULL_SOUND_PATH[] = "sound/jumpstats/unreal.mp3";
new const String:LEETJUMP_RAMPAGE_RELATIVE_SOUND_PATH[] = "*jumpstats/unreal.mp3";
new const String:LEETJUMP_DOMINATING_FULL_SOUND_PATH[] = "sound/jumpstats/outstanding.mp3";
new const String:LEETJUMP_DOMINATING_RELATIVE_SOUND_PATH[] = "*jumpstats/outstanding.mp3";
new const String:PROJUMP_FULL_SOUND_PATH[] = "sound/jumpstats/excellent.mp3";
new const String:PROJUMP_RELATIVE_SOUND_PATH[] = "*jumpstats/excellent.mp3";
...
{
	name = "KZ JumpStats",
	author = "1NutWunDeR (KINZ Editon)",
	description = "",
	version = VERSION,
	url = "https://steamcommunity.com/id/8RUNO1/"
};
...
{
	new client = GetClientOfUserId(GetEventInt(Event, "userid"));	
	//Coloquei esse if para tirar o jump do ct
...
{	
	...
	{	
		...
		if( g_js_fJump_Distance[client] >= 215.0 ) {		
			SetHudTextParams( -1.0, 0.70, 6.0, 0, 255, 159, 255, 0, 0.0, 0.1, 0.3 );
			ShowSyncHudText(client, g_hHudSync, "LJ Distance: %f\nMaxspeed: %.0f\nPrestrafe: %f\nStrafes: %d\nSync: %d%%", g_js_fJump_Distance[client], g_js_fMax_Speed_Final[client], g_js_fPreStrafe[client], strafes, sync );
		}
		...
		{		
			...
			if (prestrafe)
				PrintToChat(client, "%t", "ClientLongJump1", PINK, DARKGREY, GRAY, g_js_fJump_Distance[client],LIMEGREEN,strafes,GRAY, LIMEGREEN, g_js_fPreStrafe[client], GRAY,LIMEGREEN,g_js_fMax_Speed_Final[client],GRAY,LIMEGREEN, fJump_Height,GRAY,LIMEGREEN, sync,PERCENT,GRAY,sBlockDist);			
			else
				PrintToChat(client, "%t", "ClientLongJump2", PINK, DARKGREY, GRAY, g_js_fJump_Distance[client],LIMEGREEN,strafes,GRAY, LIMEGREEN, g_js_fPreStrafe[client], GRAY,LIMEGREEN,g_js_fMax_Speed_Final[client],GRAY,LIMEGREEN, fJump_Height,GRAY,LIMEGREEN, sync,PERCENT,GRAY,sBlockDist);			
			
			
			
			PrintToConsole(client, "        ");
			PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a LongJump [%i Strafes | %.3f %s | %.0f Max | Height %.1f | %i%c Sync]%s",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], szVr,g_js_fMax_Speed_Final[client],fJump_Height,sync,PERCENT,sBlockDistCon);
			...
			{			
				if (g_js_fJump_Distance[client] >= g_dist_leet_lj && g_js_fMax_Speed_Final[client] > 275.0)	
				{
					...
					//client		
					PrintToConsole(client, "        ");
					PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a LongJump [%i Strafes | %.3f %s | %.3f Max | Height %.1f | %i%c Sync]%s",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client],szVr, g_js_fMax_Speed_Final[client],fJump_Height,sync,PERCENT,sBlockDistCon);
					PrintToConsole(client, "%s", szStrafeStats);		
					if (prestrafe)					
	...
	{		
		...
		if (g_js_fJump_Distance[client] >= g_dist_good_multibhop && g_js_fJump_Distance[client] < g_dist_pro_multibhop)	
		{
			...
			PrintToConsole(client, "        ");
			PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a MultiBhop [%i Strafes | %3.f Pre | %3.f Max | Height %.1f | %s Bhops | %i%c Sync]",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], g_js_fMax_Speed_Final[client], fJump_Height,szBhopCount,sync,PERCENT);				
			PrintToConsole(client, "%s", szStrafeStats);
		}	
		else
			//pro?
			...
			{	
				...
				//Client
				PrintToConsole(client, "        ");
				PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a MultiBhop [%i Strafes | %.3f Pre | %.3f Max |  Height %.1f | %s Bhops | %i%c Sync]",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], g_js_fMax_Speed_Final[client], fJump_Height,szBhopCount,sync,PERCENT);				
				PrintToConsole(client, "%s", szStrafeStats);					
	...
	{		
		...
		{					
			...
			if (g_js_fJump_Distance[client] >= g_dist_good_dropbhop && g_js_fJump_Distance[client] < g_dist_pro_dropbhop)	
			{
				...
				PrintToConsole(client, "        ");
				PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a DropBhop [%i Strafes | %.3f Pre | %.3f Max | Height %.1f | %i%c Sync]",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], g_js_fMax_Speed_Final[client],fJump_Height,sync,PERCENT);						
				PrintToConsole(client, "%s", szStrafeStats);
			}	
			else
				//pro
	...
	{						
			...
			else
			{
				...
				if (g_js_fJump_Distance[client] >= g_dist_good_weird && g_js_fJump_Distance[client] < g_dist_pro_weird)	
				{
					...
					PrintToConsole(client, "        ");
					PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a WeirdJump [%i Strafes | %.3f Pre | %.3f Max | Height %.1f | %i%c Sync]",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], g_js_fMax_Speed_Final[client],fJump_Height,sync,PERCENT);						
					PrintToConsole(client, "%s", szStrafeStats);	
				}	
				//pro?
				else
	...
	if (ground_frames < 11 && g_js_Last_Ground_Frames[client] > 10 && fGroundDiff == 0.0 && fJump_Height <= 67.0 && !g_js_bDropJump[client] && g_js_fPreStrafe[client] > 200.0)
	{
		...
		{				
			...
			PrintToConsole(client, "        ");
			PrintToConsole(client, "GREY[PINKJUMPSTATSGREY] %s jumped %0.4f units with a Bhop [%i Strafes | %.3f Pre | %.3f Max | Height %.1f | %i%c Sync]",szName, g_js_fJump_Distance[client],strafes, g_js_fPreStrafe[client], g_js_fMax_Speed_Final[client],fJump_Height,sync,PERCENT);						
			PrintToConsole(client, "%s", szStrafeStats);
		}	
		else
			//pro?
		...
		if (rank < 21 && rank < g_js_BhopRank[client])
		{
			...
			{ 
				...
				{ 					
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Bunnyhop Top 20! [%.3f units]", szName, rank, g_js_fPersonal_Bhop_Record[client]);
				}
			}
		}
	}
		...
		if (rank < 21 && rank < g_js_DropBhopRank[client])
		{
			...
			{ 
				...
				{ 			
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Drop-Bunnyhop Top 20! [%.3f units]", szName, rank, g_js_fPersonal_DropBhop_Record[client]);
				}
			}
		}
	}
		...
		if (rank < 21 && rank < g_js_MultiBhopRank[client])
		{
			...
			{ 
				...
				{ 					
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Multi-Bunnyhop Top 20! [%.3f units]", szName, rank, g_js_fPersonal_MultiBhop_Record[client]);
				}
			}
		}
	}
		...
		{			
			...
			{ 
				...
				{ 		
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Longjump 20! [%i units block/%.3f units jump]", szName, rank, g_js_Personal_LjBlock_Record[client],g_js_fPersonal_LjBlockRecord_Dist[client]);
				}
			}	
		}
	}
		...
		{			
			...
			{ 
				...
				{ 		
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Longjump 20! [%.3f units]", szName, rank, g_js_fPersonal_Lj_Record[client]);
				}
			}
		}
	}
		...
		{			
			...
			{ 
				...
				{ 		
					...
					PrintToConsole(i, "GREY[PINKJUMPSTATSGREY] %s is now #%i in the Weirdjump 20! [%.3f units]", szName, rank, g_js_fPersonal_Wj_Record[client]);
				}
			}	
		}
	}
