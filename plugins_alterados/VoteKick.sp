...
public OnPluginStart()
{
	...
	g_hMinPlayers 	= CreateConVar("votekick_minplayers","2","Minimum of Players in server",FCVAR_NOTIFY);

	RegConsoleCmd("sm_votekick",PugCmdVoteKick, "Votação para Kikar um Jogador.");
	RegConsoleCmd("say .votekick",PugCmdVoteKick, "Votação para Kikar um Jogador.");
}
...
{
	if( ( GetUserFlagBits( iClient ) & ADMFLAG_RESERVATION || GetUserFlagBits( iClient ) & ADMFLAG_ROOT ) ) { 
		if(iClient) {
			GetPlayers(g_iPlayers);
			...
			{
				PrintCenterText(iClient,"{green}[DETRAN] {lightgreen}Precisa de mais de 2 jogadores para fazer a votação.");
			}
			else
			{
		...
		CPrintToChat(iClient, "{green}[DETRAN] {lightgreen}Somente {blue}Vips{green}, {lightgreen}Mods {green}e {red}Admins {lightgreen}podem usar esse comando!");
	}
}
