...
public Action MSG(Handle timer, any client)
{
	if(IsValidClient(client))
		CPrintToChat(client, "{green}[pNr] {lightgreen}%t", "Restored");
}
...
{           
	if(GetUserFlagBits( client ) & ADMFLAG_RESERVATION || GetUserFlagBits( client ) & ADMFLAG_ROOT){
		...
		{
			CPrintToChat(client, "{green}[pNr] {lightgreen}%t", "Plugin Disabled");
		...
		if(GetClientDeaths(client) == 0 && GetClientFrags(client) == 0 && CS_GetMVPCount(client) == 0)
		{
			...
			{
				CPrintToChat(client, "{green}[pNr] {lightgreen}%t", "Score 0");
				return Plugin_Continue;
			}
		}
		...
		{
			CPrintToChat(client, "{green}[pNr] {lightgreen}%t", "No Money", cost);
			return Plugin_Continue;
		}
		...
		if(GetConVarInt(hPublic) == 1)
		{
			...
			{
				CPrintToChatAll("{green}[pNr] {lightgreen}%t", "Player Reset Red", name);
			}
			else if(GetClientTeam(client) == 3)
			{
				CPrintToChatAll("{green}[pNr] {lightgreen}%t", "Player Reset Blue", name);
			}
			else
			{
				CPrintToChatAll("{green}[pNr] {lightgreen}%t", "Player Reset Normal", name);
			}
		}
		else
		{
			CPrintToChat(client, "{green}[pNr] {lightgreen}%t", "You Reset");
		}
	} else {
		CPrintToChat(client, "{green}[pNr] {lightgreen}Somente {blue}Vips{green}, {lightgreen}Mods {green}e {red}Admins {lightgreen}podem usar esse comando!");
	}          
...
{                           
	...
	if (args != 1)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_resetplayer <name or #userid>");
		return Plugin_Continue;
	}
	...
	ShowActivity2(client, "[pNr] ", "%t", "Reset Score of", target_name);
	return Plugin_Continue;
}
...
{                           
	...
	if (args != 4)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_setscore <name or #userid> <Kills> <Deaths><Stars>");
		return Plugin_Continue;
	}
	...
	ShowActivity2(client, "[pNr] ", "%t", "Set Score", target_name);
	return Plugin_Continue;
}
...
{                           
  	if (args != 6)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_setscore <name or #userid> <Kills> <Deaths><Assists><Stars><Points>");
		return Plugin_Continue;
	}
...
{                           
	...
	if (args != 2)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_setpoints <name or #userid> <points>");
		return Plugin_Continue;
	}
	...
	ShowActivity2(client, "[pNr] ", "%t", "Set Points of", target_name, points);
	return Plugin_Continue;
}
...
{                           
	...
	if (args != 2)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_setassists <name or #userid> <assists>");
		return Plugin_Continue;
	}
	...
	ShowActivity2(client, "[pNr] ", "%t", "Set Assists of", target_name, assists);
	return Plugin_Continue;
}
...
{                           
	...
	if (args != 2)
	{
		ReplyToCommand(client, "{lightgreen}[pNr] sm_setstars <name or #userid> <stars>");
		return Plugin_Continue;
	}
	...
	ShowActivity2(client, "[pNr] ", "%t", "Set Stars of", target_name, stars);
	return Plugin_Continue;
}
