...
public void OnPluginStart()
{
	...
	c_HRadius		= CreateConVar("sm_stuck_horizontal_radius", 	"70", 	"Horizontal radius size to fix player position", _, true, 10.0);
	...
	c_SpawnCheck	= CreateConVar("sm_stuck_spawncheck",			"1", 	"Check if players are stuck after they spawn (1 to enable, 0 to disable)", _, true, 0.0, true, 1.0);
...
public Action StuckCmd(int client, any args)
{
	...
	{
		CPrintToChat(client, "{green}[DETRAN] {default}Você tem que está {purple}vivo para usar esse comando!", client);
		return;
	}
	
	if (GetConVarInt(c_RoundTime) >= 1) {
		if (g_bRoundRestrict) {
			CPrintToChat(client, "{green}[DETRAN] {purple}%i {default}segundos se passaram, você tem que esperar até o próximo round para usar o {lightred}!stuck", GetConVarInt(c_RoundTime));
			return;
		}
	}
	...
	if (GetConVarInt(c_Limit) > 0)
	{
		...
		{
			CPrintToChat(client, "{green}[DETRAN] {default}Você deve esperar {purple}%i {default}segundos antes de usar esse comando de novo.", RoundFloat(g_fTime[client] - GetGameTime()));
			return;
		}
	...
	{
		PrintToChat(client, "{green}[DETRAN] {default}Unstuck já está em progresso, {purple}%i {default}checagens completas até agora.", g_iStuckCheck[client]);
		return;
	}
}
...
{	
	...
	if(GetVectorDistance(vecOrigin, vecOriginAfter, false) < 8.0) // Can't move
	{
		...
		if(g_iStuckCheck[client] < 2 && g_iStuckCheck[client] != -1) {
			PrintToChat(client, "{green}[DETRAN] {default}Aparentemente você não está preso...");
			...
			PrintToChat(client, "{green}[DETRAN] {default}Você deve estar {bluegrey}livre {default}agora.");
			//Enable controls
			int flags = GetEntityFlags(client) & ~FL_ATCONTROLS;
			SetEntityFlags(client, flags);
			g_iStuckCheck[client] = -1;
...
stock void TryFixPosition(int client, float Radius, float pos_Z, float DegreeAngle)
{
	...
	if (g_iStuckCheck[client] == -1) {
		PrintToChat(client,"{green}[DETRAN] {default}Alguma coisa deu errado, se você ainda está preso, tente {lightred}!stuck {default}de novo ou chame um {purple}admin{default}.");
		if (DelayTimer[client] != null)
		{
			KillTimer(DelayTimer[client]);
			DelayTimer[client] = null;
