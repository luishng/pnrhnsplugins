...
public void OnPluginStart()
{
	...
	g_Cvar_ChatPrefix = CreateConVar("sm_gloves_chat_prefix", "[DETRAN]", "Prefix for chat messages");
...
{
	if (IsValidClient(client) && GetUserFlagBits( client ) & ADMFLAG_RESERVATION || GetUserFlagBits( client ) & ADMFLAG_ROOT )
	{
		CreateMainMenu(client).Display(client, MENU_TIME_FOREVER);
	} else {
		CPrintToChat(client, "{green}[DETRAN] {lightgreen}Somente {blue}Vips{green}, {lightgreen}Mods {green}e {red}Admins {lightgreen}podem usar esse comando!");
	}
