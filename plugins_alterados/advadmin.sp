...
{
	CVAR_ADMINS		= CreateConVar("sm_advadmin_admins",		"0",	"Settings of !admins command, 0 - disable, 1 - show fake message, 2 - show online admins", _, true, 0.0, true, 2.0);
	CVAR_ANNOUNCE	= CreateConVar("sm_advadmin_announce",		"0",	"Join announce, 0 - disable, 1 - simple announce, 2 - announce with country name", _, true, 0.0, true, 2.0);
	CVAR_INVALID	= CreateConVar("sm_advadmin_invalid",		"0",	"Invalid given item will show for all players just for fun, 0 - disable, 1 - enable", _, true, 0.0, true, 1.0);
	...
	//-----//
	RegAdminCmd("sm_team",			CMD_Team,			ADMFLAG_BAN,		"Set the targets team");
	RegAdminCmd("sm_swap",			CMD_Swap,			ADMFLAG_BAN,		"Swap the targets team");
	RegAdminCmd("sm_spec",			CMD_Spec,			ADMFLAG_BAN,		"Set the targets team to spectator");
	RegAdminCmd("sm_scramble",		CMD_Scramble,		ADMFLAG_BAN,		"Scramble the teams by scores");
	//-----//
	RegAdminCmd("sm_respawn",		CMD_Respawn,		ADMFLAG_BAN,		"Respawning the targets");
	...
	RegAdminCmd("sm_sb",		CMD_SpawnBall,		ADMFLAG_GENERIC,	"Spawn one ball on your aim position");
